...
async def show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if bots:
        ...
        text += i18n.t('هیچ رباتی ثبت نشده.', _lg)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "back":
        ...
        await context.bot.send_message(
            ...
            text=i18n.t('به منوی نمایندگی بازگشتید.', _lg),
    ...
    if action == "activate":
        ...
        await query.answer(i18n.t('فعال شد ✅', _lg) if new_active else i18n.t('غیرفعال شد ❌', _lg))
    ...
    if action == "token":
        ...
        try:
            await context.bot.send_message(
                chat_id=_chat_id,
                text=_prompt, reply_markup=cancel_keyboard( lang=_lg), parse_mode="HTML",
            )
        except Exception:
            try:
                await query.message.reply_text(
                    _prompt, reply_markup=cancel_keyboard( lang=_lg), parse_mode="HTML",
                )
            except Exception:
                pass
        return
    ...
    if action == "restart":
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(i18n.t('🔙 بازگشت', _lg), callback_data="agbot:cbot:back")],
            ]),
        )
        return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if ":" not in text or len(text) < 30:
        await update.message.reply_text(i18n.t('فرمات توکن نامعتبر است.', _lg))
        return True
    try:
        ...
        await update.message.reply_text(i18n.t('توکن نامعتبر است. لطفا توکن صحیح ارسال کنید.', _lg))
        return True
    agent_db.add_customer_bot(agent_id, text, username)
    context.user_data.pop(UD_STATE, None)
    from AgentBot.keyboards import main_menu_keyboard
    await update.message.reply_text(
        f"{i18n.t('✅ ربات @', _lg)}{_escape(username)}{i18n.t(' با موفقیت ثبت شد!', _lg)}",
        parse_mode="HTML",
    )
    ...
    await update.message.reply_text(
        i18n.t('📊 <b>پانل نمایندگی</b>\nاز منوی زیر گزینه مورد نظر را انتخاب کنید.', _lg),
        reply_markup=main_menu_keyboard( lang=_lg), parse_mode="HTML",
    )
    return True
