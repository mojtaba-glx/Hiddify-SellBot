...
async def revoke_user_link_on_panel(panel_user_uuid: str, server_id: int) -> Dict[str, Any]:
    """Revoke / regenerate user subscription links."""
    server = database.get_server_by_id(server_id)
    if not server:
        return {"new_uuid": ""}
    return await multi_panel.revoke_user_link(server, panel_user_uuid)
...
async def disable_user_on_panel(panel_user_uuid: str, server_id: int) -> bool:
    server = database.get_server_by_id(server_id)
    if not server:
        return False
    await multi_panel.disable_user(server, panel_user_uuid)
    return True


async def enable_user_on_panel(panel_user_uuid: str, server_id: int) -> bool:
    server = database.get_server_by_id(server_id)
    if not server:
        return False
    await multi_panel.enable_user(server, panel_user_uuid)
    return True


async def delete_user_on_panel(panel_user_uuid: str, server_id: int) -> bool:
    server = database.get_server_by_id(server_id)
    if not server:
        return False
    await multi_panel.delete_user(server, panel_user_uuid)
    return True


async def get_user_configs(panel_user_uuid: str, server_id: int) -> List[str]:
    server = database.get_server_by_id(server_id)
    if not server:
        return []
    try:
        configs = await multi_panel.get_user_configs(server, panel_user_uuid)
