...
- ساخت اینباند از لینک کانفیگ (vless/vmess/hysteria2/trojan) با تشخیص دامنه و سوال پورت
- همگام‌سازی یوزرها روی همه اینباندها + دکمه همگام‌سازی جدید
- فیکس TLS اینباند (سرتیفیکت پنل) و هندل پورت اشغال توسط پنل

تمرکز نسخه `5.0.0`:

- اضافه شدن پنل **X-UI (3x-ui)** به عنوان نوع سرور جدید در کنار هیدیفای و مارزبان
- حالت هوشمند چنداینبانده: `0`=همه اینباندها، `1`=تک، `1,2,3`=چندتایی — UUID یکسان در همه
- تجمیع لینک هوشمند و کانفیگ‌ها از همه اینباندها + جمع ترافیک و تمدید/غیرفعال همزمان
- نمایش نام کاربر در پنل X-UI به جای UUID و انتشار نام به همه اینباندها
- فیکس‌های ادمین: نمایش X-UI، منوی ویرایش X-UI، دکمه بازگشت، آدرس پنل با base path
